...
<div class="faq-section section-padding subscribe-details" data-aos="fade-up" data-aos-delay="200">
        ...
        <div class="col-sm-6 col-md-5 col-lg-5">
            <div>
                ...
                <div class="card-body">
                    <form action="{{ route('freelancer-confirm-subscription') }}" class="needs-validation" id="checkout_form" method="post" enctype="multipart/form-data">
                        {{ csrf_field() }}
                    @php $no = 1; @endphp
                        ...
                        <div class="lebel">
                           ...
                           <label for="opt1-{{ $payment }}" >{{ $payment }} 
                           @if($payment == 'wallet')
                                @auth
                                ...
                                @else
                                    @if(Auth::guest())
                                        ...
                                        ({{ Helper::price_format($allsettings->site_currency_position, Auth::guard('freelancer')->user()->earnings, $currency_symbol, $multicurrency) }})
                               
                                        @else
                                        ({{ __('Please log in to use wallet') }})
                                        @endif
                                    @else
                                        ({{ __('Please log in to use wallet') }})
                                    @endif
